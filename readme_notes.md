...
[x]get a new Django project set up. Don't forget the `.` at the end of the command to `startproject`!
...
[x]After you run this, your directory structure should look like this:
...
[x]To generate an app in your django_music project (so that you have something analogous to `contacts` in the Uptact assignment), you want to run the following (where <name_of_app> is a name you can choose) in your repo:
...
[x]and your directory structure would look like:
...
[x]Once you have your app set up, be sure to add it to the `INSTALLED_APPS` list in `settings.py`.
...
[x]🦋 It can be tricky to get these installed, but don't be afraid to try. Let an instructor know if things go sideways and you need some help!
